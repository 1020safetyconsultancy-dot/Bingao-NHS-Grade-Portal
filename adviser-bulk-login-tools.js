(function(){
  ...
  function adviserLearners(){
    if(!window.me || me.role!=='adviser') return [];
    return (window.students||[]).filter(s=>!s.adviser_user_id || s.adviser_user_id===me.user_id);
  }
  ...
  window.bulkCreateMissingStudentLogins=async function(){
    if(me?.role!=='adviser') return alert('Adviser access required.');
    ...
    alert(`${creds.length} student login(s) created.\n\nUse “Print Generated Slips” to print all newly created login slips.${errors.length?`\n\n${errors.length} account(s) had issues.`:''}`);
  };
  ...
  window.prepareAllStudentLoginSlips=async function(){
    ...
    const msg=`Prepare printable login details for all ${list.length} assigned learner(s)?\n\n${missing} missing login(s) will be created.\n${active} existing login password(s) will be RESET so a new temporary password can be printed.\n\nExisting passwords will stop working.`;
    ...
    if(errors.length) alert(`${errors.length} learner(s) could not be prepared. The successful slips were still generated.`);
  };

  window.bulkPrintStudentLoginSlips=function(){
    const creds=window.bulkStudentCredentials||[];
    if(!creds.length) return alert('No generated login details are available to print. Create missing logins or prepare all slips first.');
  ...
  function addBulkButtonsToStudents(){
    ...
    group.innerHTML=`<button class="btn primary" onclick="bulkCreateMissingStudentLogins()">Bulk Create Logins</button><button class="btn outline" onclick="bulkPrintStudentLoginSlips()">Bulk Print Slips</button>`;
